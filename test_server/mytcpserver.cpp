...
margin: 0 auto;\
    ...
    if (rowsChecking(QString(arr)) == 0) {
        mTcpSocket->write(headFormArr + slashNArr + formArr);
    }
    else if (rowsChecking(QString(arr)) == 1) {
...
int MyTcpServer::rowsChecking(QString row) {
    if (row.startsWith("GET") && (row.contains("HTTP") && row.contains("lt=") && row.contains("id="))) {
        if (row.contains("username") && row.contains("place") && row.contains("time")) {
            return int(Enums::EnumTypeRequest::Approved);
        }
        return int(Enums::EnumTypeRequest::Form);
    }
    return int(Enums::EnumTypeRequest::Error);
}
